...
1. Replace crypto library with pure-js rusha library which makes ablaevent-node work with Cloudflare Workers in Next.js edge runtime.
...
1. Make sure shutdown waits for pending promises to resolve. Fixes a problem with using Ablaevent Node in serverless environments.

# 2.5.0 - 2023-02-15

1. Removes shared client from `ablaevent-node`, getting rid of some race condition bugs when capturing events.
...
2. Minimum Ablaevent version requirement is 1.38
...
1. You can now evaluate feature flags locally (i.e. without sending a request to your Ablaevent servers) by setting a personal API key, and passing in groups and person properties to `isFeatureEnabled` and `getFeatureFlag` calls.
